...
function displayChildren(response) {
    try {
        ...
        if (result.length === 0) {
            ...
            $("#myChildrenTitle").html(translations.MyAssistantProfessors);
        } else {
            ...
            $("#myChildrenTitle").html(translations.MyAssistantProfessors + " (" + (iCount) + ")");
        }
...
function fillDropDown(response) {
    ...
    dropdown.empty();
    dropdown.append('<option selected="true" disabled>' + translations.SelectResponsible + '</option>');
