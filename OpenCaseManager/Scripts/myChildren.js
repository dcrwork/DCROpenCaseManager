...
function SearchChildren() {
    var oEle = document.getElementById('responsibleny');
    var initials = "$(loggedInUser)";
    var user = window.App.user;
    if (oEle.selectedIndex > 0) {
        initials = oEle.options[oEle.selectedIndex].id;
    }
    var query = {
        CaseManagerInitials: initials,
        CaseContent: "Løbende*"
    }

    API.service('Services/SearchChildren', query, 60000)
        .done(function (response) {
        ...
        if (result.length === 0) {
            ...
            for (i = 0; i < result.length; i++) {
                if (!result[i].IsClosed) {
                    ...
                    childList += result[i].CaseID + ",";
                    iCount += 1;
                }
            }
            $("#myChildrenTitle").html("Mine Sager (" + (iCount) + ")");
        }
...
function getChildInstanceHtml(item) {
    var childLink = "../Child?id=" + item.CaseID;

    var returnHtml = "<tr class='trStyleClass' id='trchild" + item.CaseID + "'>";
    returnHtml += "<td><span class='dot dotGrey' title='...'></span></td>";
    returnHtml += "<td style='display:none;'></td>";
    returnHtml += "<td>" + '' + "</td>";
    returnHtml += "<td><a href='" + childLink + "'>" + GetName(item.ChildName) + "</a></td>";
    returnHtml += "<td>" + FormatCPR(item.ChildCPR) + "</td>";
...
function fillDropDown(response) {
    ...
    $(JSON.parse(response)).map(function () {
        dropdown.append('<option id="' + this.sAMAccountName + '">' + this.Name + '</option>');
    });
}
