    ...
    public class ServicesController : ApiController
    {
        ...
        public IHttpActionResult ExecuteEvent(dynamic input)
        {
            try
            {
                ...
                Common.AddJournalHistory(instanceId, trueEventId, null, "Event", title, DateTime.Now, _manager, _dataModelManager);
        ...
        public IHttpActionResult ExecuteGlobalEvents(List<GlobalEvents> globalEvents)
        {
            try
            {
                ...
                foreach (var globalEvent in globalEvents)
                {
                    ...
                    Common.AddJournalHistory(globalEvent.InstanceId, globalEvent.TrueEventId.ToString(), null, "Event", globalEvent.EventTitle, DateTime.Now, _manager, _dataModelManager);
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(eventsXml))
                {
                    try
                    {
                        ...
                        Common.AddJournalHistory(instanceId, eventDetails.Rows[0]["Id"].ToString(), null, "Event", eventDetails.Rows[0]["Title"].ToString(), DateTime.Now, _manager, _dataModelManager);
        ...
        public IHttpActionResult UpdateInstanceDCRXML(dynamic input)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(instanceId) && !string.IsNullOrWhiteSpace(dcrXML))
                {
                    ...
                    var oldGraphTable = _manager.SelectData(_dataModelManager.DataModel);
                    string oldGraph = oldGraphTable.Rows[0][DBEntityNames.Instance.DCRXML.ToString()].ToString();

                    XmlDocument oldXmlDocument = new XmlDocument();
                    oldXmlDocument.LoadXml(oldGraph);

                    XmlDocument newXmlDocument = new XmlDocument();
                    newXmlDocument.LoadXml(dcrXML);


                    XmlNode oldSpecification = oldXmlDocument.SelectSingleNode("//dcrgraph/specification");
                    XmlNode newSpecification = newXmlDocument.SelectSingleNode("//dcrgraph/specification");


                    XmlNode oldRoot = oldXmlDocument.SelectSingleNode("//dcrgraph");
                    oldRoot.RemoveChild(oldSpecification);

                    XmlNode newNode = oldXmlDocument.ImportNode(newSpecification, true);
                    oldRoot.InsertBefore(newNode, oldRoot.FirstChild);

                    string newDCRxml = oldXmlDocument.OuterXml;

                    _dataModelManager.GetDefaultDataModel(Enums.SQLOperation.UPDATE, DBEntityNames.Tables.Instance.ToString());
                    _dataModelManager.AddParameter(DBEntityNames.Instance.DCRXML.ToString(), Enums.ParameterType._xml, newDCRxml.ToString());
                    _dataModelManager.AddFilter(DBEntityNames.Instance.Id.ToString(), Enums.ParameterType._int, instanceId.ToString(), Enums.CompareOperator.equal, Enums.LogicalOperator.none);
                    _manager.UpdateData(_dataModelManager.DataModel);
