...
namespace OpenCaseManager.Managers
{
    public class DocumentManager : IDocumentManager
    {
        ...
        public string AddDocument(string instanceId, string fileType, string givenFileName, string fileName, string eventId, IManager manager, IDataModelManager dataModelManager)
        {
            ...
            var childId2 = Common.GetInstanceChildId(manager, dataModelManager, instanceId);

            if (!isDocumentAdded)
            {
                Common.AddDocument(givenFileName, fileType, fileLink, instanceId, childId2.ToString(), false, DateTime.Now, manager, dataModelManager);
            }
            return filePath;
        }
        ...
        public Tuple<string, string> AddDocument(string instanceId, string fileType, string givenFileName, string fileName, string eventId, bool isDraft, DateTime eventDateTime, IManager manager, IDataModelManager dataModelManager)
        {
            try
            {
                ...
                string filePath = AddDocumentChecks(instanceId, fileType, givenFileName, fileName, eventId);
                var childId = Common.GetInstanceChildId(manager, dataModelManager, instanceId);
                string documentId = Common.AddDocument(givenFileName, fileType, fileLink, instanceId, childId.ToString(), isDraft, eventDateTime, manager, dataModelManager);
